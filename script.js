...
function startDemoGame() {
  ...
  state.demoMode = true;
  addLog("main", `Mode jury activé. Code final : ${FINAL_CODE}.`);
  beginGame();
}
...
function beginGame() {
  const enterGame = () => {
    ...
    renderHud();
    showDialogue("Dispatch réseau", introDialogue, () => {
      startTimer();
    });
  };
...
function getLockedRoomMessage(roomId) {
  ...
  return puzzle
    ? `Salle verrouillée : résous d'abord ${puzzle.kicker}.`
    : "Salle verrouillée.";
}
...
function showPuzzleModal(id) {
  ...
  renderHintButton();
  els.submitButton.textContent = id === "final" ? "Stabiliser" : "Valider";
...
function startAudioAfterFirstCompris() {
...
function openEasterPuzzle(id, number) {
  ...
  els.modalBody.innerHTML = `
    <p>${puzzle.question}</p>
    <input type="text" name="easterAnswer" aria-label="Réponse easter egg" placeholder="Ta réponse">
  `;
  els.feedback.textContent = "";
  els.hintButton.classList.remove("hidden");
  renderHintButton();
  els.submitButton.textContent = "Valider";
...
function renderHintButton() {
  ...
  if (state.hintCooldownRemaining > 0) {
    ...
    els.hintButton.textContent = `Indice disponible dans ${state.hintCooldownRemaining} s`;
    return;
  }
  ...
  els.hintButton.textContent = state.activeEasterPuzzle
    ...
    : state.demoMode
      ? "Indice"
      : `Indice (-${HINT_COOLDOWN_SECONDS} s)`;
}
...
function showHint() {
  ...
  if (state.activeEasterPuzzle) {
    ...
    if (!puzzle) return;
    els.feedback.textContent = puzzle.hint || "Observe bien l'image cachée.";
    startHintCooldown();
    return;
  }
  ...
  const hints = puzzle.hints && puzzle.hints.length > 0
    ? puzzle.hints
    : ["Observe les éléments importants de la salle."];
...
function submitPuzzle() {
  ...
  if (id === "final" && !isCorrect) {
    ...
    els.feedback.textContent = `Le tableau refuse le code ${typedCode || "vide"}. Vérifie l'ordre des chiffres.`;
    return;
  }

  if (!isCorrect) {
    els.feedback.textContent = id === "final"
      ? "Le tableau refuse le code. Vérifie les fragments obtenus."
      : "Ce n'est pas encore stable. Essaie un autre réglage.";
    return;
  }
  ...
  els.feedback.textContent = `Correct. Fragment obtenu : ${puzzle.digit}.`;
  closeModal();
  showDialogue(
    "Ce que nous venons d'apprendre",
    `${puzzle.endDialogue}\n\nBravo, nous avons obtenu un <strong>fragment de code : ${puzzle.digit}</strong>.`
  );
}
...
function completeMainPuzzle(id, options = {}) {
  ...
  if (!state.solved.has(id)) {
    ...
    const prefix = options.fromJury ? "Mode jury : " : "";
    addLog("main", `${prefix}${puzzle.kicker} résolue. Fragment ${puzzle.digit}.`);
  }
...
function solveCurrentJuryPuzzle() {
  ...
  showDialogue(
    "Mode jury",
    `${puzzle.endDialogue}\n\n<strong>Fragment réseau affiché : ${puzzle.digit}</strong>.`
  );
}
...
function triggerJuryVictory() {
  ...
  addLog("main", "Victoire déclenchée.");
  winGame();
}
...
function triggerJuryDefeat() {
  ...
  addLog("main", "Mode jury : défaite déclenchée par expiration du timer.");
  renderHud();
  loseGame();
}
...
function grantAllFragmentsForJury() {
  ...
  if (addedBonus) {
    addLog("easter", "Fragments cachés révélés.");
  }
...
function submitEasterPuzzle() {
  ...
  if (!isEasterAnswerCorrect(answer, puzzle)) {
    ...
    els.feedback.textContent = puzzle.hint
      ? `Ce n'est pas la bonne réponse. Indice : ${puzzle.hint}`
      : "Ce n'est pas la bonne réponse. Observe bien l'easter egg.";
    return;
  }
  ...
  if (!state.solvedEasterEggs.has(id)) {
    state.solvedEasterEggs.add(id);
    state.code[puzzle.index] = puzzle.digit;
    addLog("easter", `${puzzle.label || "Bonus"} ${puzzle.order || "?"}/3 résolu. Fragment ${puzzle.digit}.`);
  }

  els.feedback.textContent = `Correct. Fragment bonus obtenu : ${puzzle.digit}.`;
  renderHud();
  closeModal();
  showDialogue(
    "Easter egg résolu",
...
function winGame() {
  ...
  const progressLabel = state.demoMode
    ? "<strong>Mode jury</strong> présentation guidée"
    : `<strong>${formatClock(state.secondsLeft)}</strong> temps restant`;
  ...
  showEnd(
    ...
    `
      ...
      <span class="victory-report">
        <span><strong>${solvedMain}/4</strong> fragments réseau</span>
        <span><strong>${solvedBonus}/3</strong> fragments cachés</span>
...
function showEnd(kicker, title, text) {
  ...
  els.endScreen.classList.toggle("defeat-screen", title !== "Victoire");
...
function renderHud() {
  els.timer.textContent = state.demoMode ? "DÉMO" : formatClock(state.secondsLeft);
...
function updateGuide() {
  ...
  const prefix = state.demoMode ? "Mode jury : chrono arrêté. " : "";
...
function updateJuryPanel() {
  ...
  els.juryLesson.textContent = juryMessages[state.currentRoom] || "Message jury : explique ce que cette salle apprend au joueur.";
  ...
  els.jurySolveCurrent.textContent = mainPuzzlesSolved
    ...
    : state.solved.has(id)
      ? `Réafficher ${puzzle.kicker}`
      : `Résoudre ${puzzle.kicker}`;
...
function addLog(type, message) {
  ...
  label.textContent = isEaster ? "Bonus" : "Réseau";
...
function selectRow(rank, options) {
  return `
    <label class="sort-row">
      <span>${rank}</span>
      <select name="rank-${rank}" aria-label="Rang ${rank}">
        ${options.map((option) => `<option value="${option}">${option || "Choisir une filière"}</option>`).join("")}
      </select>
    </label>
  `;
}

function availabilityRow(name, label, options = ["", "Très faible", "Dépend du vent", "Réactif", "Pilotable pour les pics", "Important et stable"]) {
  return `
    <label class="sort-row availability-row">
      <span class="availability-label">${label}</span>
      <select name="${name}" aria-label="${label}">
        ${options.map((option) => `<option value="${option}">${option || "Choisir"}</option>`).join("")}
      </select>
    </label>
  `;
}
